...
- **アクセス制御：**
    - クエリパラメーターにuserIdが存在しない場合、ログイン画面(`/`)にリダイレクトする
    - パスパラメータの id（施設ID）が数値変換不能な場合、または該当する施設データが DB に存在しない場合は 404 Not Found ページを表示する

- **← 施設詳細に戻るボタンを押下時：**
    - 更新を行わずに施設詳細画面（`/facilities/[id]`）へ遷移する

- **更新ボタン押下時:**
    - 入力内容を保存後、自動的に施設詳細画面（`/facilities/[id]`）へ遷移する
...
### 画面構成
- 完了を伝える文章
- 登録した施設詳細画面(`/facilities/[id]`)に戻るボタン
- 施設一覧(`/facilities`)に戻るボタン
...
- **動的ボタン表示:**
    - クエリパラメータに facilityId が存在する場合のみ、「今登録した施設の詳細を見る」ボタンを表示する

- **パラメータ維持・URL構築:**
    - 遷移用のリンク（詳細画面・一覧画面）を生成する際、クエリパラメータに含まれる prefectureId および userId を引き継いでURL（buildQuery）を組み立てる
...
- **アクセス制限：**
    - urlのクエリパラメーターにuserIdが含まれていない場合、自動的にログイン画面(`/`)にリダイレクトする
    - パスパラメータの id（施設ID）が数値でない場合、または DB 内に対象施設が存在しない場合は 404 Not Found ページを表示する

- **初期値の設定：**
    - 各入力項目には、DB から取得した現在の施設データを初期表示する（住所・HP URLが未設定の場合は空文字）
    - 都道府県の選択肢(selectは)は DB から取得し、現在の設定値を初期選択状態とする

- **更新ボタン押下時：**
    - フォーム送信時、Server Action (updateFacility) にてバリデーション（施設名の入力・都道府県IDの数値化チェック）を実施後、Prisma を経由して DB を更新する
    - 更新完了時、または「キャンセル」ボタン押下時は、クエリパラメータ（prefectureId, userId）の値を維持したまま施設詳細画面（/facilities/[id]）へ遷移する
...
- **初期値の設定：**
    - 各入力項目には DB から取得した現在の訪問ログデータを初期表示する（評価 rating、利用料金 fee、コメント comment）
    - 訪問日（visitDate）には当日の日付（YYYY-MM-DD）がデフォルトとしてセットされる
    - 処理に必要なパラメータ（visitId, userId, facilityId, prefectureId）は (input type="hidden") としてフォーム内で保持する

- **バリデーション：**
    - 訪問日 (visitDate): 必須項目（required）、日付形式
    - 評価 (rating): 必須項目（required）,、1〜5の選択肢から指定
    - 利用料金 (fee): 任意項目、0以上の数値（min="0"）
    - コメント (comment): 任意項目

- **更新ボタン押下時：**
    - フォーム送信時、Server Action 側で基本パラメータ（visitId, userId, comment）のバリデーションおよび DB 上での所有者再チェック（二重チェック）を実施する
    - クエリパラメータ（prefectureId, userId）を維持したまま施設詳細画面（/facilities/[id]）へリダイレクトする
